...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        order_exists = False
        attempt = 1
        while attempt <= 5:  # if there's an asynchronous delay between stripe and the order creation, wait and try again
            try:
        ...
        if order_exists:
            ...
            return HttpResponse(
                    content=f'Webhook received: {event["type"]} | SUCCESS: Verified order exisits in database',
                    status=200)
        else:
            order = None
            try:
